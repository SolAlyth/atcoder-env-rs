...
impl Solver {
    ...
    pub fn print(self) {unsafe{let s=&*self.v.get();if s.len()!=0{println!("{}", s);}}}
}
impl<T: Display> Shl<T> for &Solver {type Output=Self;fn shl(self,rhs:T)->Self::Output{unsafe{let s=&mut*self.v.get();let t=format!("{}",rhs);if s.len()!=0{s.push(' ');}s.push_str(&t);}self}}
#[allow(non_camel_case_types)] pub struct end;
impl Shl<end> for &Solver {type Output=();fn shl(self,_:end)->Self::Output{if cfg!(debug_assertions)||self.b{unsafe{let s=&mut*self.v.get();println!("{}",s);s.clear();}}()}}
*/
